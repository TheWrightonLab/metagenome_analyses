...
#changed the paths to the 2 databases
#	
...
for($i=0; $i<=$len1; $i++)
{
	...
	chomp($kegg[$i]);
	@scaff1=split("\t", $kegg[$i],3);
	#print OUT "$scaff1[2]\n";
	if (exists $hash{$scaff1[1]}) 		# if the element in the hash exists in the array
	{	
		print "$scaff1[0]\t$scaff1[1]\t$scaff1[2]\t$hash{$scaff1[1]}\n";
	} 
}
close(IN1);
#close(OUT);
...
for($j=0; $j<=$len2; $j++)
{
	#print OUT "$key $hash{$key}\n";
	chomp($uni[$j]);
	@scaff2=split("\t", $uni[$j],3);
	#print OUT "$scaff1[2]\n";
	if (exists $hash1{$scaff2[1]}) 		# if the element in the hash exists in the array
	{	
		print "$scaff2[0]\t$scaff2[1]\t$scaff2[2]\t$hash1{$scaff2[1]}\n";
	} 
}
close(IN3);
#close(OUT1);
